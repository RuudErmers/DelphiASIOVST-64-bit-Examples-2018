...
// Changed by RE... Public instead of protected
    ...
    function GetChunk(const Data: Pointer; const IsPreset: Boolean = False): Integer;  virtual;  // changed to virtual by RE
    ...
    function GetProgramName: AnsiString; virtual; // virtual changed by RE
    ...
    function SetChunk(const Data: Pointer; const ByteSize: Integer; const IsPreset: Boolean = False): Integer; virtual; // changed to virtual by RE
    ...
    procedure SetProgramName(const newName: AnsiString);  virtual;  // changed to virtual by RE
...
public
  ...
  public
    ...
    function Add: TVstPlugIn;overload;
{ RE.. to add inherited plugins }
    function Add(pluginclass:TCollectionItemClass): TVstPlugIn;overload;
...
procedure WriteLog(S:string);begin end;
...
function TVstPlugIns.Add(pluginclass:TCollectionItemClass): TVstPlugIn;
VAR item:TCollectionItem;
begin
  item := pluginclass.Create(Self);
  Added(item);
  result:=TVstPlugIn(item);
end;
...
begin
 ...
 if Assigned(FVstEffect) then
   try Result := FVstEffect.EffectFlags except end;
end;
...
begin
 ...
 {$ENDIF}
  ...
  // update blocksize
  if (Collection<>NIL) and (Collection.Owner is TCustomVstHost)
...
begin
 ...
 {$ENDIF}
 OutputDebugString('in Close 1');

 if FActive then VstDispatch(effClose);
 OutputDebugString('in Close 2');
 FActive       := False;
 FPlugCategory := vpcUnknown;
 OutputDebugString('in Close 3');
...
begin
 if not Active
  then raise EVstHostException.Create('Plugin not active!');
  WriteLog('TCustomVstPlugIn.ShowEdit 1');
 if Control = nil
  then raise EVstHostException.Create('Control must exist!');
 if (effFlagsHasEditor in FVstEffect.EffectFlags) and (FGUIStyle = gsDefault) then
  begin
   if not FEditOpen then
    begin
     r:=GetRect;
     WriteLog('TCustomVstPlugIn.ShowEdit 2');

     Control.Width:=r.Width;
     Control.Height:=r.Height;
     WriteLog('TCustomVstPlugIn.ShowEdit 3');
     EditOpen(Control.Handle);
     WriteLog('TCustomVstPlugIn.ShowEdit 4');
...
//  else raise EVstHostException.Create('Editor is already open!');
  ...
  WriteLog('TCustomVstPlugIn.ShowEdit Bijna End');
  ...
  begin
   with TLabel(FGUIElements[FGUIElements.Add(TLabel.Create(Control))]) do
    begin
     ...
     Height := 16; Alignment := taCenter; Left := 2; Top := 2 + (i * (Height+4));
    end;
   with TLabel(FGUIElements[FGUIElements.Add(TLabel.Create(Control))]) do
    begin
     Name := 'LbV' + IntToStr(I); Tag := i; Anchors := [akTop, akRight];
     Parent  := Control; Alignment := taCenter; AutoSize := False;
     Height  := 16; Left := Control.Width - Left - 72-50; // RE: -50
     Alignment := taCenter; Width := 65; Top := 2 + (i * (Height+4));
    end;
   ...
   with TScrollBar(FGUIElements[j]) do
    begin
     ...
     Height := 16; Top := 2 + i * (Height+4); Name := 'SB' + IntToStr(I);
     Left := MaxParamWidth + 2; Width := Control.Width - Left - 72 - 50; // RE: -50
...
begin
 ...
 with TScrollBar(Sender) do
  try
   ...
   s:=' ('+inttostr(round(Parameter[Tag]*127))+')';
...
//  FVstDllHandle := LoadLibraryEx(PWideChar(DLLFileName), 0, DONT_RESOLVE_DLL_REFERENCES);
...
begin
 if not FileExists(FileName) then if Filename='' then exit
                                                 else raise EVstHostException.Create(RStrBankFileDoesNotExist);
